...
以下是截至 2026-09-11 的生产编排说明；GitHub 中的代码更新不会自动发布 ADS，也不会自动修改 DolphinScheduler 工作流。

正式日常入口为 `吉客云_每日数据总同步`。因销售导出依赖人工更新 cURL/commonVerify，当前09:20计划保持停用，只允许更新认证材料后手工执行。销售单查询、销售单明细账、采购入库和货权采购统一滚动刷新最近30天；正向全链路保留最近7天逐日可靠刷新。销售明细、采购入库和库存完成后刷新品牌映射并回填，金额链与品牌链都完成后执行统一质量闸门，只有质量检查通过才刷新 BI ADS。库存同步在写入 ODS 前统一剥离“得物-虚拟仓”“金华佰川综合仓”“义乌自营仓”；销售 ODS 保留原始数据，销售 DWD 只剥离发货仓库为“得物-虚拟仓”的记录。
...
预检默认会用“渠道列表”发起一个只读的第一页请求，提前识别 401 登录态失效，避免工作流启动后四条源任务同时失败；仅排障时可追加 `--skip-live` 跳过实时检查。
...
`吉客云_每周180天历史对账` 已创建但默认保持离线。首次人工验证长周期运行时间和平台承载能力后，才使用 `--enable-weekly` 启用每周日02:00计划。`BI_ADS_每周版本维护` 是独立的低峰维护流程，周六02:00执行预检、分批清理和严格验收；销售与库存均只保留最近两个 ready 物理版本，`ads_publish_batch` 批次审计历史不删除。使用 `--enable-retention` 才会开启该计划。日常及180天对账都在 ADS 发布完成后执行只读存储健康检查；物理压缩或原子换表仍只允许人工维护，不进入定时任务。原三条独立日常入口及一次性 `BI_ADS_客户历史全量补数` 均下线并保留历史版本。

`吉客云_历史库存月末快照` 的目标计划为每月2日06:00。正式抓取前先用 `历史库存_web.py --preflight` 发起一次只读单页请求；登录态失效时立即结束，不再等待三轮大任务重试。默认暂停计划，确认历史库存 cURL 可用后，用 `--enable-history-inventory` 恢复定时；也可先手工启动验证。

质量检查结果写入 `ops.dq_check_result`。销售品牌的稳定下游入口为 `dwd.销售单明细账_品牌补全`；当前为兼容视图，避免每日复制四百多万行数据。日常品牌任务只回填最近60天，历史全量仅在离线周对账任务中执行；货权采购、金额DWD、抖音进口超市补全、天猫补全、品牌回填、质量闸门和ADS严格串行，避免多个大写任务同时压 MySQL。ADS 使用远端后台短连接触发、Dolphin 本地轮询发布批次，避免 SSH 中途抖动误判长时间构建失败。
...
安装后先配置 `.env.example` 中列出的环境变量，并把最新登录态放入本机/Worker 的 `curl/` 目录。项目不自动读取 `.env` 文件，生产环境应由 DolphinScheduler 或容器注入变量。吉客云 Web 请求默认使用 `https://web.jackyun.com`；若后续再次切换域名，可通过 `JKY_WEB_BASE_URL` 统一覆盖，无需逐个修改脚本。
...
安装或更新扩展后，切换到吉客云或 BSCM 页面会自动开始监听，图标显示 `ON`；切换到其他网页后会立即停止监听。扩展弹窗中的按钮仅用于临时暂停或恢复当前业务标签页，正常情况下无需点击：

- 吉客云完成验证并触发 `startExcelExport`；
- BSCM 点击正向全链路导出；
- BSCM 货权页面点击查询或导出。

扩展只自动监听当前激活的吉客云/BSCM 标签页和四个白名单接口，不会在浏览其他网站时继续监听，也不自动点击页面或处理验证码。
...
`dwd.销售单查询_进口超市上海仓补全` 是实体表，字段名称和顺序与 `ods.销售单查询` 完全一致，不增加字段。公司作为供应商，“进口超市上海仓”收入按货权转移采购含税金额补全，不使用消费者支付 GMV。只有物流、平台订单、采购单和 SKU 全部唯一匹配，且明细合计与采购主单总额一致时，才补全 `ods.销售单查询` 和 `ods.销售单明细账` 的原有空/0金额字段，DWD 直接继承补全结果。
